...
> **GF-P8G2 — the settlement principal is REAL once identity is threaded (AMENDED 2026-06-12).** `settle_turn`'s `ctx.principal` is the runtime's ambient `self.principal` — and that ambient identity now actually lands: via the `principal=` ctor passthrough on `AnthropicAgent` (forwarded to the `AgentRuntime` base) or via `set_principal` post-build (the `SessionManager` threading, tenancy §B.4). Before this fix every resident agent settled with an ANONYMOUS principal and consumer billing callbacks that key on `s.principal` silently skipped (a live smoke billed ZERO turns; the consumer injected the request principal inside its deduction callback as a workaround — that workaround dies). `set_principal` timing: the new identity applies from the NEXT settlement onward. Specs: `tests/interface/pricing_cost/test_pricing_cost_settlement_identity.py`.

> **GF-P7G1 — `on_usage_report` subscribers propagate to sub-agent children (AMENDED 2026-06-12, ratified D2).** `SubAgentTool.run` registers the parent's `on_usage_report` subscribers on each child **at child build time, via the public `on_usage_report` registration path** — so a child turn's `TurnSettlement` reaches the same in-process billing subscriber the consumer registered once on the root, stamped with the child's `agent_id`, the parent's id as `parent_agent_id`, and the (spawn-inherited) principal — named once the parent is named (G2). Propagation is **recursive by construction**: the child's callback list now contains the propagated entries, so the child's own `SubAgentTool` propagates them again to grandchildren at THEIR spawn. **Timing contract:** propagation at build time is the contract — subscribers registered on the parent AFTER a child was already built do **not** retro-attach to that child; the next spawn picks them up (the same semantics the consumer's `_propagate_to_subagents` builder-wrap workaround had — that workaround dies). This changes NOTHING about the aggregation decision above: each child still settles independently; the `SettlementAggregator` (I9) remains the cumulative roll-up and is NOT wired here (future work). Specs: `tests/interface/pricing_cost/test_pricing_cost_settlement_identity.py`.
