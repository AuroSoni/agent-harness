...
> **AMENDED (2026-06-10):** the round-2 review (see AMENDMENTS.md) supersedes parts of this document. Items marked ⚠️AMENDED below have updated outcomes; AMENDMENTS.md wins on conflict.
...
| `Scope` | ~~`agent_base/storage/scope.py`~~ DELETED | ~~`@dataclass(frozen=True) Scope{tenant, subject}` + `Scope.of(principal)`, `is_unscoped()`~~ | ⚠️AMENDED (2026-06-10): **`Scope`, `Scope.of()`, `set_scope()`, `ScopedConfigAdapter.wrap` DELETED** (O2). The one public seam is `adapter.for_principal(principal)`; adapters read `principal.tenant/.subject` directly and ignore claims. |
...
| `HookOutcome` | `agent_base/core/hooks/outcome.py` | `{decision: "proceed"\|"block"="proceed", reason, update: Any\|None, additional_context, events: list[MetaBody]}` | ⚠️AMENDED (2026-06-10): **`switch_profile` field DELETED** (O7) — profile switching is the imperative `ctx.switch_profile(name)` only, "last call in the chain wins, applied once post-composition." **agent-loop-hooks** owns. Specialized: `TurnStartOutcome`, `EndTurnOutcome`. Composition: decision most-restrictive-wins; `update` chains in registration order; `additional_context`/`events` concatenate. |
...
| `FrontendCallView` | `agent_base/streaming/meta.py` | `@dataclass(frozen=True) {tool_use_id, tool_name, input: dict}` | ⚠️AMENDED (2026-06-10): R6 REVERTED — the field is **`tool_use_id`**, NOT `cid` (B7). `cid` means exactly one thing: the pause-level reply key (`ToolReply.cid == MetaEnvelope.correlation_id`). FE contract: "reply with the envelope's correlation_id; attribute per-call results by tool_use_id." **streaming** owns. |
...
| `ErrorCode` | `agent_base/core/errors.py` | closed `str` Enum, **8 members**: `PROVIDER_OVERLOADED, RATE_LIMITED, PROVIDER_TIMEOUT, PROVIDER_STATUS, CONTEXT_OVERFLOW, TOOL_FAILED, ABORTED, INTERNAL` | ⚠️AMENDED (2026-06-10): **trimmed to 8 members** (O6). Dropped: `PROVIDER_BAD_REQUEST`, `PROVIDER_AUTH`, `AUTH`, `VALIDATION` (collapse into `PROVIDER_STATUS` + `details`/`native_code`), `CREDITS_EXHAUSTED` (consumer-side; use `details` or a registered body). **core** owns the taxonomy; **streaming** (`ErrorDelta.code`) and **providers** (`ProviderError.code`) import it. Exactly one enum. |
...
| `ToolContext` (`ctx`) | `agent_base/tools/context.py` (shipped, extended) | shipped `{run_id, tool_call_id, attempt, replay_reason, idempotency_key, _once_store}` + `once()`; **+ `sandbox: Sandbox\|None`, `principal: SessionPrincipal\|None`, `emit(body, *, correlation_id=None, expects_reply=False)`, `call_frontend_tool(name, input)`, `emit_capped(text, *, max_chars=25_000)`** | ⚠️AMENDED (2026-06-10): public seam is `call_frontend_tool(name, input) -> list[ContentBlock]` (I4) — `await_external(cid)` demoted to runtime-internal; `emit` signature gains `correlation_id`/`expects_reply` and its **unwired default RAISES** `RuntimeError("ctx.emit not available in this execution context")` (B8); gains `emit_capped`/`emit_capped_bytes` budgeting on ctx (I5). **tools** owns the type; the **loop** populates them at call-time. |
| `ToolResultEnvelope` | `agent_base/tools/tool_types.py` (shipped ABC, extended) | ABC with `for_context_window() -> list[ContentBlock]`, `for_conversation_log() -> ToolLogProjection`; mutation surface `with_text`/`append_text` + `from_blocks`/`from_text` + readers, with **CONCRETE default implementations on the ABC** | ⚠️AMENDED (2026-06-10): mutation surface = `with_text`/`append_text`/`from_blocks`/`from_text` with **concrete ABC defaults** (custom subclasses inherit working mutation); **`with_blocks`/`from_image` DEFERRED** (O11). **tools** owns. This is the `after_tool` `update=` payload (§3-R10). |
...
| `MemoryStore` | `agent_base/memory/base.py` (**Protocol only**) | `retrieve(ctx: HookContext, user_message) -> MemoryContribution`, `update(ctx: HookContext, log, stop_reason) -> MemoryUpdate` | ⚠️AMENDED (2026-06-10): **Protocol only — `BaseMemoryStore` ABC DELETED** (O5/O13). `retrieve`/`update` take a `HookContext` + extras (the bespoke `MemoryRetrieveContext`/`MemoryUpdateContext` DELETED); `retrieve` returns `MemoryContribution(blocks, placement: Literal["user_suffix","system_suffix"])` (homed `agent_base/memory/base.py`). **memory** owns. |
| `Provider` | `agent_base/core/provider.py` (shipped, expanded) | Protocol: `generate`, `generate_stream(sink=…)`, `build_request`, `parse_response`, `translate_event`, `classify_error -> ProviderError`, `sanitize_chain`, `plan_stream_abort`, `extract_tool_calls`, `collect_api_files`, `name`, `default_model()`, **`make_llm_config(loaded) -> LLMConfig`**, `token_estimator`; carries a **`RetryPolicy{max_retries, base_delay}`** value | ⚠️AMENDED (2026-06-10): GAINS `RetryPolicy` (per-provider budgets; runtime stops threading scalars) + `make_llm_config(loaded: dict\|LLMConfig\|None)`; LOSES `llm_config_cls()`/`coerce_llm_config()`/ctor default (O12). `ProviderError` carries `code: ErrorCode` directly — `ProviderErrorKind` + `PROVIDER_KIND_TO_ERROR_CODE` DELETED (O5; `classify_error` returns `ProviderError{code, native_code, retriable}`). **providers** owns. |
...
| `Profile` | `agent_base/profiles.py` | `@dataclass(frozen=True) {name, tools, frontend_tools, system_prompt, tail}` — **no `ui_capabilities`** (2026-06-10 amendment: consumer FE payloads move to the `on_profile_changed` observer hook, hooks §2.3a; library auto-emits only the minimal `ProfileChanged(profile)`) | **agent-loop-hooks** owns; persisted as `AgentConfig.active_profile: str\|None`. |
| `Settlement` / `TurnSettlement` | `agent_base/core/cost.py` | the once-per-turn billing fact carrying `{agent_id/agent_uuid, run_id, parent_agent_id, principal, turn_usage, turn_cost, model, step_count}` (turn-level only) | ⚠️AMENDED (2026-06-10): **turn-level ONLY — `cumulative_*` REMOVED** (served by `SettlementAggregator` + `AgentResult`); `to_dict()` and the `UsageReport` body serialize **`tenant`/`subject` only — never `claims`** (B2; the in-process `.principal` object keeps the full principal). `_Settler` class → module function `settle_turn(ctx, steps) -> TurnSettlement` (O14). **core** + **pricing-cost** co-own — see §3-R11. |
...
| `Serializable` + `SCHEMA_VERSION_KEY` | `agent_base/core/serializable.py` | documented `.to_dict()` **convention** + `_v` stamping via `_stamp()`; a single `CORE_SCHEMA_VERSION` | ⚠️AMENDED (2026-06-10): `Serializable` **demoted to a documented convention** — no `runtime_checkable` Protocol (O15). ONE `CORE_SCHEMA_VERSION` stamped by `_stamp()`; **per-entity `SCHEMA_VERSION` ClassVars DROPPED**. (storage's `LIBRARY_SCHEMA_VERSION` for DDL stays a separate axis.) **core** owns. |
| `AwaitTable` / `AwaitRecord` / `AwaitReason` / `Join` | `agent_base/await_table/{table,types}.py` (shipped, extended) | `AwaitRecord` gains `principal: SessionPrincipal\|None` + `reason: str = "frontend_tool"`; `AwaitTable.resolve(cid, results, *, principal=None) -> Disposition` consults the injected policy; `AwaitTable.cancel(cid, *, principal=None) -> Disposition`; `ResumeOutcome` dataclass | ⚠️AMENDED (2026-06-10): `resolve` consults the **injected `PrincipalPolicy`** (one policy shared with `SessionManager.get_or_create`; I1/R7) — may return `REJECTED`; new **`cancel(cid, *, principal=None)`** closes one record + cancels its future as abort (I6; `reconcile=False` escape hatch DELETED). **`AwaitReason` → `reason: str` open vocabulary** with four documented string constants (O9). **`ResumeOutcome` is a dataclass** `{status: Literal["resumed","aborted"], results: list[ContentBlock]}` carrying results (B3; homed `agent_base/await_table/types.py`). **relay-await** owns. |
...
sequenceDiagram
    ...
    ST-->>C: FE renders FrontendCallView(tool_use_id, tool_name, input)
...
> **⚠️AMENDED (2026-06-10):** the public frontend-tool seam is `ctx.call_frontend_tool(name, input) -> list[ContentBlock]` (the diagram's `await_external(cid)` is now runtime-internal; I4). `AwaitInput` is the ONLY await frame — the legacy `awaiting_frontend_tools` MetaDelta dual-emit + the codec's legacy mapping are deleted (B5). The `InlineRelayRegistry` bridge (+ `cid==agent_uuid` + `_relay_mode`) is deleted (O3). `switch_profile` rides imperative `ctx.switch_profile(name)` only (O7).
...
> **⚠️AMENDED (2026-06-10):** `_rearm_pending_await()` splits — it ALWAYS re-opens the cid record + re-enters the parked state, but emits `AwaitInput` **only when NO inbound reply is in hand** (the reply-triggered cold path resolves with zero re-emit) (B4). `AwaitInput` is the only await frame (B5). `await_external` stays runtime-internal; the public seam is `call_frontend_tool` (I4).
...
| **R30** | `generate_stream` signature: `DeltaSink` vs `(queue, stream_formatter)` | providers (`sink: DeltaSink`), streaming (owns the wire; must define `DeltaSink`) | Provider depends on streaming actually shipping `DeltaSink`. | **streaming MUST ship `DeltaSink`** (`agent_base/streaming/wire.py` or `sink.py`) with `emit(StreamDelta)` + `emit_meta(MetaBody)`. providers' `generate_stream(sink=…)` is canonical. The `(queue, stream_formatter)` pair is a one-major back-compat shim the runtime wraps into a `DeltaSink`. If streaming declines `DeltaSink`, the shim becomes permanent — but the reconciled decision is streaming ships it.<br>**⚠️AMENDED (2026-06-10):** the `(queue, stream_formatter)` / `run_stream(msg, queue, formatter)` back-compat shim is DELETED (G0); `stream()` ships at Rung 1 as a single-subscriber `AsyncIterator[StreamItem]` (replay/fan-out = Rung 2 behind `from_seq`) (I3). |
...
| **R32** | Memory fires via existing `on_turn_start`/`on_turn_end`, not a new hook | memory, hooks (owns the catalog, LOCKED) | Memory must not add a hook to the LOCKED catalog. | **Confirmed**: memory rides the existing `on_turn_start` (retrieve) / `on_turn_end` (update) boundaries; the runtime derives `MemoryRetrieveContext`/`MemoryUpdateContext` from the hook `HookContext`. No new hook. `MemoryStore` remains the place for storage logic; a consumer wanting to veto a recall registers an ordinary `on_turn_start` hook.<br>**⚠️AMENDED (2026-06-10):** `retrieve`/`update` take a **`HookContext`** + extras (the bespoke `MemoryRetrieveContext`/`MemoryUpdateContext` DELETED); `MemoryStore` is **Protocol only** (`BaseMemoryStore` ABC DELETED); `retrieve` returns `MemoryContribution(blocks, placement)` (O5/O13). |
...
| `HookOutcome` **shape** | ⚠ | `switch_profile` field missing in most → add it (R5). ⚠️AMENDED (2026-06-10): field DELETED — imperative `ctx.switch_profile(name)` only (O7). |
...
| `ctx` (`ToolContext`) | ⚠ | Needs `sandbox`/`principal`/`emit`/`await_external`/`media` added (R3). ⚠️AMENDED (2026-06-10): public seam is `call_frontend_tool(name, input)` (await_external runtime-internal); `emit` gains `correlation_id`/`expects_reply` + raising unwired default; `emit_capped*` added (I4/I5/B8). |
| `ToolResultEnvelope` as `update=` | ⚠ | contract says `ToolResult` → `ToolResultEnvelope` (R10). |
| `FrontendCallView` field names | ⚠ | `tool_use_id`/`name` vs `cid`/`tool_name` → `cid`/`tool_name` (R6). ⚠️AMENDED (2026-06-10): R6 REVERTED — field is `tool_use_id`, not `cid` (B7); `name`→`tool_name` stands. |
...
| **D — Rollback channel** | streaming (Fork F-1), hooks, abort | **A**: `RollbackDelta` on content channel. **B**: `Rollback` as `MetaBody`. | **B** (R13): `Rollback` is a `MetaBody`; `RollbackDelta` is a deprecated alias. `ctx.emit(Rollback(...))` → meta channel.<br>**⚠️AMENDED (2026-06-10):** the `RollbackDelta` alias is **DELETED OUTRIGHT** (G0) — removed from the `StreamDelta` taxonomy; `Rollback` MetaBody only, no codec mapping (O3). | If A: content stream conflates UI-only signals with LLM output; consumers special-case. |
...
| **F — Cross-worker `submit` (Rung 2)** | session-control (Fork B2) | Forward (RPC/pub-sub) vs return `NOT_FOUND`/`MISDIRECTED`. | **Reserve `Disposition.MISDIRECTED` (→ 421) now** to avoid a later enum break; Rung-1 behavior is single-process (in-process dict + LRU + idle-TTL), so the choice is deferred but the value is reserved.<br>**⚠️AMENDED (2026-06-10):** keep the `Disposition.MISDIRECTED` enum member (with a `# Rung 2` comment) but **DROP the 421 row from `DISPOSITION_HTTP_STATUS` until Rung 2** (O4). | Adding `MISDIRECTED` later is a public-enum break; reserving now is free. |
| **G — Cost settlement delivery** | pricing (Fork P-1) | **A**: `UsageReport` MetaEnvelope is the single delivery (+ `on_usage_report` sugar). **B**: a new `on_turn_settled` lifecycle hook. | **A** (matches §3 correlation-header attribution; avoids a 2nd delivery path for one datum; the §2 catalog is LOCKED so B would require adding a row). `AgentResult.settlement` is the awaited-caller copy.<br>**⚠️AMENDED (2026-06-10):** the catalog gained a sibling observer hook — **`on_profile_changed`** (observe + emit only; `ProfileChangedContext{old_profile, new_profile, source ∈ {restore, session_default, hook_switch}, is_initial}`; fires after the swap, for every source; no switch capability). Library auto-emits minimal `ProfileChanged(profile)`. (Cost delivery itself is unchanged.) | If B: adds a LOCKED-catalog row; mild double-delivery tension (same object, not double extraction). |
...
| **J — Tools envelope spelling** | tools (Fork, contract §7) | **A**: `ToolResultEnvelope.from_blocks(...)` classmethod. **B**: standalone `StructuredEnvelope` dataclass. | **A** as primary (one public class), **B** as a thin alias; if B wins, `from_blocks` returns a `StructuredEnvelope`. Same `_StructuredEnvelope` mechanics either way.<br>**⚠️AMENDED (2026-06-10):** the public `StructuredEnvelope` alias is **DELETED** (`_StructuredEnvelope` stays private); `from_blocks`/`from_text` on `ToolResultEnvelope` are the only public spelling (O3). | Purely ergonomic; both back the same projection. |
...
- **Fork (cid allocation):** keep A (runtime-minted opaque cid) as design, B (cid==agent_uuid) as one-major bridge — already aligned; note it's decided jointly with streaming (which owns `correlation_id`).
  - **⚠️AMENDED (2026-06-10):** the `InlineRelayRegistry` bridge — including the `cid==agent_uuid` derivation and the `_relay_mode` no-op attr — is DELETED (G0). Runtime-minted opaque cid (variant A) is the only path; the legacy `awaiting_frontend_tools` MetaDelta dual-emit AND the codec's legacy mapping are deleted — `AwaitInput` is the only await frame (B5/O3). `call_frontend_tool` mints the cid, emits `AwaitInput`, returns the reply, NEVER splices (I4).
