"""Inline context compaction for the LiteLLM agent loop."""
...
class CompactionController:
    ...
    def find_safe_boundary(
        ...
        while boundary_idx > 0:
            ...
            if message.role == Role.USER and not self._message_has_tool_results(message):
                return boundary_idx
            boundary_idx -= 1
    ...
    def prepare_summary_messages(self, older_messages: list[Message]) -> list[Message]:
        ...
        for message in copy.deepcopy(older_messages):
            if message.role == Role.ASSISTANT:
                message.content = [
                    block for block in message.content if not isinstance(block, ThinkingContent)
                ]
    ...
    def _build_summary_config() -> Any:
        ...
        return LiteLLMConfig(thinking=None)
